...
function mime_from_ext($f){
  ...
  return match($ext){ 'mp4'=>'video/mp4','webm'=>'video/webm','ogg'=>'video/ogg','mov'=>'video/quicktime', default=>'video/mp4' };
}
function stream_range($file,$mime,$name){
  ...
  if(isset($_SERVER['HTTP_RANGE']) && preg_match('/bytes=(\d+)-(\d*)/i',$_SERVER['HTTP_RANGE'],$m)){
    ...
    fseek($fp,$start);
    $left=$len;
    while($left>0 && !feof($fp)){ $chunk=fread($fp,min(65536,$left)); if($chunk===false) break; echo $chunk; $left-=strlen($chunk); @ob_flush(); flush(); }
    fclose($fp); exit;
  } else {
...
try{
  $st=$pdo->prepare("SELECT id, title, filename, code_ecole FROM videos WHERE id=:id AND code_ecole=:ec LIMIT 1");
  $st->execute([':id'=>$id,':ec'=>$ecole]);
  $v=$st->fetch(PDO::FETCH_ASSOC);
  if(!$v){ http_response_code(404); exit('Vidéo introuvable'); }
  $file=resolve_media_path((string)$v['filename'], 'videos');
  if(!$file){ http_response_code(404); exit('Fichier manquant'); }
  $name=$v['title'] ?: ('video-'.$v['id'].'.mp4');
  $mime=mime_from_ext($file);
  if($raw){ stream_range($file,$mime,$name); }
} catch(Throwable $e){
  error_log("view_video error: ".$e->getMessage());
...
.card{background:#fff;border:1px solid #e5e7eb;border-radius:12px;box-shadow:0 1px 2px rgba(0,0,0,.04);padding:12px;max-width:1100px;width:98%;}
  ...
  <div class="wrapper">
    <div class="card">
      <div class="d-flex align-items-center justify-content-between mb-2">
        <div class="fw-semibold text-truncate me-2"><?=h($v['title'] ?: 'Vidéo')?></div>
        <a class="btn btn-sm btn-outline-secondary" href="?id=<?=$v['id']?>&raw=1">Télécharger</a>
      </div>
      <video controls preload="metadata">
        <source src="?id=<?=$v['id']?>&raw=1" type="<?=h($mime)?>">
