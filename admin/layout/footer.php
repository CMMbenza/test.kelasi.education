<footer class="footer-wrap-layout1">
  <div class="copyright">© Copyrights <a href="#">BoomRang Group</a> 2025.
    All rights reserved. Designed by <a href="#">Site officiel</a></div>
</footer>
...
<style>
  .notif-fab{
    ...
    z-index:1055; border:none; outline:none;
  }
  .notif-fab:hover{ filter:brightness(1.05); }
  .notif-fab i{ font-size:20px; }
  .notif-badge{
    position:absolute; top:-6px; right:-6px;
    background:#dc3545; color:#fff; font-weight:700;
    font-size:.75rem; min-width:22px; height:22px; line-height:22px;
    border-radius:999px; text-align:center; padding:0 6px;
    box-shadow:0 0 0 2px #fff;
  }
  /* modal à droite façon drawer */
  .modal-right .modal-content{ min-height:100vh; border:0; border-radius:0; }
  .modal-right{ margin:0 0 0 auto; }
</style>
...
(function(){
  ...
  function fetchFeed(){
    ...
    $.getJSON('service/notify_feed.php', function(resp){
      if(!(resp && resp.status === 'ok')){ $('#notif-feed').html('<div class="p-3 text-danger">Erreur de chargement.</div>'); return; }
      const items = resp.items || [];
      const since = resp.since || null;
      if(since){ $('#notif-last-since').text('Depuis : ' + fmt(since)); }
